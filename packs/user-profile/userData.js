...
import {
    ...
    Image,
    StyleSheet,
    ScrollView, RefreshControl, Platform, TouchableOpacity, Pressable,
} from "react-native";
...
const Profile = () => {
    ...
    const setUserData = useCallback((userData, tokens) => {
    ...
    const handleSave = useCallback(() => {
        ...
        sendDefaultRequest(`${SERVER_AJAX_URL}/user/save_basic_settings.php`,
            data,
            navigation
        )
            .then((data) => {
                if (data.email_changed) {
                    Toast.show({
                        type: "success",
                        text1: "Письмо с подтверждением нового email отправлено, перейдите по ссылке, что бы активировать его"
                    });
                }

                setUserData(data.user, data.tokens)
            })
            .catch(() => {
            })
    ...
    const handleConfirmEmail = useCallback(() => {
        ...
        sendDefaultRequest(`${SERVER_AJAX_URL}/user/send_confirm_email.php`,
            {},
            navigation
        )
            .then(() => {
    ...
    const onRefresh = useCallback(() => {
        ...
        sendDefaultRequest(`${SERVER_AJAX_URL}/user/get_user.php`,
            ...
            .then((data) => setUserData(data.user, data.tokens))
            .catch(() => {
            })
        ...
        >
            ...
            <View>
                <View style={[styles.avatarContainer]}>
                    <View style={[styles.userContainerImageBorder, {position: "relative"}]}>
                        ...
                        <Image
                            source={
                                image.uri ? {uri: image.uri} :
                                    (user.image === "default.png" ? require("../images/other_images/userphoto.png") : {
                ...
                <View style={globalCss.inputGroup}>
                    ...
                    <View style={[styles.inputView, styles.inputContainer1]}>
                        <TextInput
                            ...
                            value={data.surname}
                            placeholder="Фамилия"
                            onChangeText={text => setData(prev => ({...prev, surname: text}))}
                        />
                    </View>
                    <Text style={styles.label}>Электронная почта</Text>
                    <View style={[styles.inputView, styles.inputContainer1, user.by_google && styles.inputInactive]}>
                        <TextInput
                            style={globalCss.input}
                            value={data.email}
                    ...
                    <View style={[styles.inputView, styles.inputContainer1, styles.bigInputContainer]}>
                        <TextInput
                    ...
                    <View style={[styles.inputView, styles.inputInactive, styles.inputContainer1]}>
                        <TextInput
                            style={globalCss.input}
                            value={
                                user.level === 0
                    ...
                    <AnimatedButtonShadow
                        styleButton={[globalCss.button, globalCss.buttonGry, styles.changePassword]}
                        shadowColor={"gray"}
                        onPress={() => {
                    ...
                    <AnimatedButtonShadow
                        styleButton={[globalCss.button, globalCss.buttonGreen, styles.saveAll]}
                        shadowColor={"green"}
                        onPress={handleSave}
                    >
